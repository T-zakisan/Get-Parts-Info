...
from PIL import  ImageGrab, Image #画像
...
class GetPartInfo(QMainWindow):
	...
	def __init__( self, path_db, path_imIC ):
		...
		# OCRの初期設定
		self.OCRtools = pyocr.get_available_tools()
		self.OCRtool = self.OCRtools[0]
	...
	def createWindow( self ):
		...
		self.FlagInput = True	#入力モード：True(テキスト), False(画像)
		self.show()	#窓を表示
	...
	def keyPressEvent(self, e):
		...
		if e.key() == Qt.Key_Escape :
			self.FlagInput = not( self.FlagInput )
			if self.FlagInput == True	: tmpClr = "#fafafa"
			else											: tmpClr = "#d8d6ff"
			self.setStyleSheet("QWidget{ background-color:"+tmpClr+";}")	#ラベルの文字色


	# クリップボードの内容を監視し、変更があれば処理" ##########
	def checkClipboard(self):
		if self.FlagInput == True :
			self.current_text = self.clipboard.text()	#テキスト入力モード時
		else : #OCR入力モード
			img = ImageGrab.grabclipboard()
			if img != None:
				self.current_text = self.OCRtool.image_to_string(
					img,
					lang = "eng",
